(async () => {
  ...
  function extensionErrorMessage(fallback = 'Extension API error') {
    try {
    ...
    return new Promise(resolve => {
      try {
        EXT.storage.local.get([key], result => {
          if (EXT.runtime?.lastError) return resolve(fallback);
    ...
    return new Promise((resolve, reject) => {
      try {
        EXT.storage.local.set({ [key]: value }, () => {
          if (EXT.runtime?.lastError) reject(new Error(extensionErrorMessage('Storage write failed')));
          else resolve();
        });
      } catch (error) {
        reject(error);
    ...
    return new Promise((resolve, reject) => {
      try {
        EXT.runtime.sendMessage(message, response => {
          if (EXT.runtime?.lastError) return reject(new Error(extensionErrorMessage('Message failed')));
          if (response?.ok === false) return reject(new Error(response.error || 'Extension request failed'));
          resolve(response);
        });
      } catch (error) {
        reject(error);
  ...
  async function readSiteBase() {
    try {
      return normalizeSiteBase(await storageGet(SITE_STORAGE_KEY, DEFAULT_BASE)) || DEFAULT_BASE;
    } catch (_) {
      return DEFAULT_BASE;
    }
  }

  let BASE = await readSiteBase();
  let STREAM_URL = '';
  let INGEST_URL = '';
  let CONTEXT_URL = '';
  let ANIME_URL = '';
  ...
  async function changeSiteBase() {
    ...
    rebuildSiteEndpoints();
    try {
      await storageSet(SITE_STORAGE_KEY, BASE);
    } catch (_) {}
  ...
  function notify(title, text) {
    sendExtensionMessage({ type: 'notify', title: String(title || ''), text: String(text || '') })
  ...
  async function gmJsonOnce(method, url, payload = null) {
  ...
  async function gmJson(method, url, payload = null) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
      try {
        ...
        addLog(`Extension background: повторюю запит ${attempt + 1}/${attempts}…`, 'warn');
  ...
  function streamProcess(payload, onPacket) {
    return new Promise((resolve, reject) => {
      const requestId = `an2n-${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const port = EXT.runtime.connect({ name: 'anime-to-yoru-stream' });
      let settled = false;
      ...
      const finish = (ok, value) => {
        ...
        settled = true;
        cleanup();
        ok ? resolve(value) : reject(value instanceof Error ? value : new Error(String(value || 'Stream error')));
      };

      const keepAliveTimer = setInterval(() => {
        try { port.postMessage({ type: 'ping', requestId, at: Date.now() }); } catch (_) {}
      }, 20000);

      const watchdog = setInterval(() => {
        const idle = Date.now() - lastPacketAt;
        const limit = lastPercent >= 95 ? 60000 : 120000;
        if (idle > limit) finish(false, new Error(`STREAM_STALLED: немає пакетів ${Math.round(idle / 1000)}с.`));
      }, 5000);
      ...
      port.onMessage.addListener(message => {
        ...
        if (message.type === 'packet' && message.packet) {
          ...
          lastPercent = Number(message.packet.percent || lastPercent || 0);
          onPacket(message.packet);
          if (message.packet.type === 'error') {
            finish(false, new Error(`CORE:${message.packet.message || 'Python core завершився з помилкою.'}`));
          } else if (message.packet.type === 'result' && message.packet.result) {
            finish(true, message.packet.result);
          }
          return;
        }
        if (message.type === 'error') {
          finish(false, new Error(message.error || 'Extension stream error'));
          return;
        }
        if (message.type === 'done' && !settled) {
          if (message.result) finish(true, message.result);
          else finish(false, new Error('STREAM_NO_RESULT: background завершив stream без фінального JSON.'));
        }
      });

      port.onDisconnect.addListener(() => {
        if (settled) return;
        finish(false, new Error(extensionErrorMessage('Extension background disconnected')));
      });

      try {
        port.postMessage({ type: 'start', requestId, url: STREAM_URL, payload });
      } catch (error) {
        finish(false, error);
      }
    });
  ...
  async function enrichAniListGenresClient(result) {
    ...
    result.meta.anilist_taxonomy_transport = 'extension-background';
  ...
  async function createNew() {
    ...
    try {
      ...
      console.error('[Anime -> YORU v3.2.0]', error);
  ...
  EXT.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message.type === 'openPanel') {
      ...
      if (!state.running) resetPanel().catch(error => addLog(error.message, 'error'));
      sendResponse?.({ ok: true });
      return true;
    }

    if (message.type === 'changeSitePrompt') {
      changeSiteBase().then(() => sendResponse?.({ ok: true })).catch(error => sendResponse?.({ ok: false, error: error.message }));
      return true;
    }

    if (message.type === 'reloadSiteBase') {
      (async () => {
        const next = await readSiteBase();
        if (next !== BASE) {
          BASE = next;
          rebuildSiteEndpoints();
          state.existingItem = null;
          updateSiteButton();
          if (state.log) addLog(`Сайт YORU → ${BASE}`, 'ok');
          if (state.panel && !state.panel.hidden && !state.running) await resetPanel();
        }
        sendResponse?.({ ok: true, base: BASE });
