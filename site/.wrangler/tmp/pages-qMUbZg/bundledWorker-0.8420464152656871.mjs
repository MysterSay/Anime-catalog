...
function tursoNormalizeAnime(raw = {}) {
  ...
  return { id: String(i.id || ""), entity: "anime", title: plainTitle(i.title || "") || "\u0411\u0435\u0437 \u043D\u0430\u0437\u0432\u0438", originalTitle: plainTitle(i.originalTitle || ""), englishTitle: plainTitle(i.englishTitle || ""), russianTitle: plainTitle(i.russianTitle || ""), aliases: String(i.aliases || ""), key: String(i.key || exactCoreTitleKey(i.originalTitle || i.title || "")), poster, banner: banner || poster, description: String(i.description || ""), status: plainTitle(i.status || "") || "\u0411\u0435\u0437 \u0441\u0442\u0430\u0442\u0443\u0441\u0443", group: plainTitle(i.group || "") || "\u0411\u0435\u0437 \u0433\u0440\u0443\u043F\u0438", addedAt: String(i.addedAt || (/* @__PURE__ */ new Date()).toISOString()), updatedAt: String(i.updatedAt || i.addedAt || (/* @__PURE__ */ new Date()).toISOString()), favorite: Boolean(i.favorite), liked: Boolean(i.liked), viewed: Math.max(0, Math.floor(Number(i.viewed) || 0)), season: Math.max(0, Math.floor(Number(i.season) || 0)), episode: Math.max(0, Math.floor(Number(i.episode) || 0)), tags: normalizeTags(i.tags || []), notes: String(i.notes || ""), links: tursoNormalizeLinks(i.links), sourceUrl: safeHttpUrl(i.sourceUrl || ""), migratedFrom: String(i.migratedFrom || "") };
}
...
function tursoSummaryAnime(raw) {
  ...
  return { id: i.id, title: i.title, poster: i.poster, status: i.status, group: i.group, addedAt: i.addedAt, favorite: i.favorite, liked: i.liked, viewed: i.viewed, season: i.season, episode: i.episode, tags: i.tags, description: i.description };
}
...
function tursoBuildOptions(items, config) {
  const statuses = new Set(TITLE_STATUS_OPTIONS), tags = /* @__PURE__ */ new Set();
  ...
  return { statuses: [...statuses], groups: groupOptions.map((g) => g.name), groupOptions, tags: [...tags].sort((a, b) => a.localeCompare(b, "uk")), statusGroups: config.statusGroups || {}, starredGroups: config.starredGroups || [], storage: "turso-libsql" };
}
...
async function tursoIngestCorePayload(env, payload) {
  if (!payload || typeof payload !== "object" || !payload.title || typeof payload.title !== "object") throw new HttpError(400, "\u041E\u0447\u0456\u043A\u0443\u0432\u0430\u0432\u0441\u044F schema-v2 JSON \u0432\u0456\u0434 Python core.");
...
async function tursoHandleAnimeApi(request, env) {
  ...
  if (request.method === "GET") {
    if (id) {
      const [raw, state2] = await Promise.all([tursoGetRaw(env, id), tursoLoadState(env)]);
...
var worker_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/version") return json({ ok: true, version: "yoru-v7.1-turso-libsql-2026-09-11", storage: "turso-libsql", pythonCoreSearch: env.CORE_SEARCH_URL || CORE_SEARCH_URL, pythonCoreProcessStream: env.CORE_PROCESS_STREAM_URL || CORE_PROCESS_STREAM_URL, pythonCoreProcessFull: env.CORE_PROCESS_FULL_URL || CORE_PROCESS_FULL_URL, progressProtocol: "ndjson-v1", coreJsonIngest: true, mergeTitles: true, deleteTitles: true, editTitles: true, extensionContext: "/api/extension/context", favoriteStorage: "turso-libsql", likedStorage: "turso-libsql", viewedStorage: "turso-libsql", seasonEpisodeStorage: "turso-libsql", groupSettings: "/api/groups", catalogSettings: "/api/catalog-settings", tagDelimiter: "dot" });
...
//# sourceMappingURL=bundledWorker-0.8420464152656871.mjs.map
