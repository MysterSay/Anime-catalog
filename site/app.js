...
const state = {
  ...
  theme: saved.theme || 'all',
  sort: saved.sort || 'added-desc',
...
function starredGroupSet() {
  return new Set((apiOptions.starredGroups || []).map(normalize));
}

function sortGroupsByPriority(groups) {
  const stars = starredGroupSet();
  return [...new Set((groups || []).filter(Boolean))].sort((a, b) => {
    const aStar = stars.has(normalize(a));
    const bStar = stars.has(normalize(b));
    if (aStar !== bStar) return aStar ? -1 : 1;
    return a.localeCompare(b, 'uk');
  });
}

function groupStatusesMap() {
  return apiOptions.statusGroups && typeof apiOptions.statusGroups === 'object' ? apiOptions.statusGroups : {};
}

function groupsWithoutAssignedStatus() {
  const assigned = new Set();
  for (const groups of Object.values(groupStatusesMap())) for (const group of groups || []) assigned.add(normalize(group));
  return (apiOptions.groups || []).filter(group => !assigned.has(normalize(group)));
}
...
function populateFilters() {
  ...
  themeFilter.innerHTML = '<option value="all">Усі теми</option>';
  ...
  const groups = sortGroupsByPriority([...(apiOptions.groups || []), ...db.map(x => x.group).filter(Boolean)]);
  ...
  const genres = [...new Set([...(apiOptions.genres || []), ...db.flatMap(x => Array.isArray(x.genres) ? x.genres : [])])].filter(Boolean).sort((a,b) => a.localeCompare(b, 'uk'));
  genres.forEach(value => genreFilter.insertAdjacentHTML('beforeend', `<option value="${escapeHtml(value)}">${escapeHtml(value)}</option>`));
  const themes = [...new Set([...(apiOptions.themes || []), ...db.flatMap(x => Array.isArray(x.themes) ? x.themes : [])])].filter(Boolean).sort((a,b) => a.localeCompare(b, 'uk'));
  themes.forEach(value => themeFilter.insertAdjacentHTML('beforeend', `<option value="${escapeHtml(value)}">${escapeHtml(value)}</option>`));

  if (![...genreFilter.options].some(o => o.value === state.genre)) state.genre = 'all';
  if (![...themeFilter.options].some(o => o.value === state.theme)) state.theme = 'all';
...
function getFiltered() {
  const items = db.filter(item => {
    ...
    const haystack = normalize([item.title, item.group, item.status, ...itemTags, ...itemTags.map(tag => `#${tag}`), ...itemGenres, ...itemThemes].join(' '));
    ...
    const matchesGenre = state.genre === 'all' || itemGenres.some(value => normalize(value) === normalize(state.genre));
    const matchesTheme = state.theme === 'all' || itemThemes.some(value => normalize(value) === normalize(state.theme));
    ...
    return matchesSearch && matchesGroup && matchesStatus && matchesGenre && matchesTheme && matchesQuick && !excludedGroup && !excludedStatus && !excludedFavorite && !excludedLiked;
  });
  ...
  items.sort((a, b) => {
    ...
    if (state.sort === 'genre-asc' || state.sort === 'genre-desc') {
      const av = [...(Array.isArray(a.genres) ? a.genres : [])].sort((x,y)=>x.localeCompare(y,'uk'))[0] || '';
      const bv = [...(Array.isArray(b.genres) ? b.genres : [])].sort((x,y)=>x.localeCompare(y,'uk'))[0] || '';
      if (!av && !bv) return a.title.localeCompare(b.title, 'uk');
      if (!av) return 1;
      if (!bv) return -1;
      const cmp = av.localeCompare(bv, 'uk') || a.title.localeCompare(b.title, 'uk');
      return state.sort === 'genre-desc' ? -cmp : cmp;
    }
    if (state.sort === 'theme-asc' || state.sort === 'theme-desc') {
      const av = [...(Array.isArray(a.themes) ? a.themes : [])].sort((x,y)=>x.localeCompare(y,'uk'))[0] || '';
      const bv = [...(Array.isArray(b.themes) ? b.themes : [])].sort((x,y)=>x.localeCompare(y,'uk'))[0] || '';
      if (!av && !bv) return a.title.localeCompare(b.title, 'uk');
      if (!av) return 1;
      if (!bv) return -1;
      const cmp = av.localeCompare(bv, 'uk') || a.title.localeCompare(b.title, 'uk');
      return state.sort === 'theme-desc' ? -cmp : cmp;
    }
    return 0;
  });
  return items;
  ...
  return `
    <article class="anime-card ${mergeMode ? 'merge-candidate-card' : ''}" style="--delay:${Math.min(index * 35, 280)}ms; --status-accent:${theme.solid}; --status-accent-glow:${theme.glow}; --status-accent-border:${theme.border}" data-id="${escapeHtml(item.id)}">
      ...
      <div class="card-body">
        ...
        ${Array.isArray(item.tags) && item.tags.length ? `<div class="card-tags">${item.tags.slice(0, 5).map(tag => `<span>#${escapeHtml(tag)}</span>`).join('')}</div>` : ''}
        <div class="card-meta">
...
function render() {
  ...
  if (items.length === 0) {
    ...
    if (db.length === 0) {
      ...
      emptyState.innerHTML = `<div class="empty-icon">◇</div><h3>Turso повернув 0 тайтлів</h3><p>${escapeHtml(sourceText)}</p>`;
    } else {
...
catalog.addEventListener('click', async e => {
  ...
  try {
    ...
    saveState();
    render();
    showToast(kind === 'favorite' ? 'Вибране збережено у Turso' : 'Улюблене збережено у Turso');
  } catch (error) {
    if (next) set.delete(id); else set.add(id);
    render();
    showToast(error.message || 'Не вдалося зберегти у Turso');
  }
});
...
themeFilter.addEventListener('change', () => { state.theme = themeFilter.value; saveState(); render(); });
sortSelect.addEventListener('change', () => { state.sort = sortSelect.value; saveState(); render(); });
...
function renderSettingsGroups() {
  ...
  const groups = sortGroupsByPriority(Array.isArray(apiOptions.groupOptions) ? apiOptions.groupOptions.map(group => group.name) : []);
  const details = new Map((apiOptions.groupOptions || []).map(group => [normalize(group.name), group]));
  const stars = starredGroupSet();
  if (!groups.length) {
    settingsGroupsList.innerHTML = '<div class="settings-empty">У Turso ще немає груп.</div>';
    return;
  }
  settingsGroupsList.innerHTML = groups.map(name => {
    const group = details.get(normalize(name)) || { id:'', name, color:'default' };
    const color = GROUP_COLOR_THEME[group.color] || GROUP_COLOR_THEME.default;
    const starred = stars.has(normalize(group.name));
    return `<article class="group-setting-row" data-group-setting="${escapeHtml(group.id || group.name)}" data-group-id="${escapeHtml(group.id || '')}" data-group-old-name="${escapeHtml(group.name)}" data-group-starred="${starred ? '1' : '0'}">
      <div class="group-setting-main">
        <button class="group-star-toggle ${starred ? 'active' : ''}" type="button" data-group-star title="Закріпити групу першою" aria-pressed="${starred ? 'true' : 'false'}">${starred ? '★' : '☆'}</button>
...
function renderSettingsStatuses() {
  if (!settingsStatusesList) return;
  const statusGroups = groupStatusesMap();
  const statuses = [...new Set([...(apiOptions.statuses || []), ...STATUS_ORDER])].filter(status => status && status !== 'Без статусу');
  const groups = sortGroupsByPriority(apiOptions.groups || []);
  if (!statuses.length) {
    settingsStatusesList.innerHTML = '<div class="settings-empty">Статусів у Turso не знайдено.</div>';
    return;
  }
  settingsStatusesList.innerHTML = statuses.map(status => {
    const selected = new Set((statusGroups[status] || []).map(normalize));
    return `<article class="status-setting-row" data-status-setting="${escapeHtml(status)}">
      <div class="status-setting-head">
        <div><span class="status-badge ${statusClass(status)}">${escapeHtml(status)}</span><small>${selected.size ? `Прив’язано груп: ${selected.size}` : 'Без окремих груп'}</small></div>
        <button class="group-setting-save" type="button" data-save-status>Зберегти</button>
      </div>
      <div class="status-group-options">
        ${groups.map(group => `<button class="status-group-option ${selected.has(normalize(group)) ? 'selected' : ''}" type="button" data-status-group="${escapeHtml(group)}" style="${groupTagStyle(group)}"><span>${escapeHtml(group)}</span></button>`).join('') || '<span class="settings-empty-inline">Груп ще немає.</span>'}
      </div>
    </article>`;
  }).join('');
}
...
async function saveGroupSetting(row) {
  ...
  const starred = row.dataset.groupStarred === '1';
  if (!name) return setSettingsStatus('Назва групи не може бути порожньою.', 'error');
  button.disabled = true;
  setSettingsStatus(`Оновлюю «${row.dataset.groupOldName}» у Turso…`, 'loading');
  try {
    const response = await fetch('/api/groups', {
      method:'PATCH', headers:{'Content-Type':'application/json',Accept:'application/json'},
      body:JSON.stringify({ id:row.dataset.groupId, oldName:row.dataset.groupOldName, name, color, starred }), cache:'no-store',
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload.error || `HTTP ${response.status}`);
...
async function saveStatusSetting(row) {
  const button = row.querySelector('[data-save-status]');
  const status = row.dataset.statusSetting || '';
  const groups = [...row.querySelectorAll('[data-status-group].selected')].map(item => item.dataset.statusGroup).filter(Boolean);
  if (!status) return;
  button.disabled = true;
  setSettingsStatus(`Зберігаю групи для «${status}»…`, 'loading');
  try {
    const response = await fetch('/api/catalog-settings', {
      method:'PATCH', headers:{'Content-Type':'application/json',Accept:'application/json'},
      body:JSON.stringify({ status, groups }), cache:'no-store',
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload.error || `HTTP ${response.status}`);
    apiOptions = { ...apiOptions, ...payload, statusGroups: payload.statusGroups || {}, starredGroups: payload.starredGroups || [] };
    setSettingsStatus(`Групи для «${status}» збережено.`, 'ok');
    renderSettingsStatuses();
  } catch (error) {
    setSettingsStatus(error.message || 'Не вдалося зберегти налаштування статусу.', 'error');
  } finally { button.disabled = false; }
}
...
settingsModal?.addEventListener('click', e => {
  ...
  if (star) {
    ...
    const next = row?.dataset.groupStarred !== '1';
    if (row) row.dataset.groupStarred = next ? '1' : '0';
    star.classList.toggle('active', next);
    star.textContent = next ? '★' : '☆';
    star.setAttribute('aria-pressed', String(next));
    return;
  }
  const statusGroup = e.target.closest('[data-status-group]');
  if (statusGroup) { statusGroup.classList.toggle('selected'); return; }
  const saveStatus = e.target.closest('[data-save-status]');
  if (saveStatus) { saveStatusSetting(saveStatus.closest('[data-status-setting]')); return; }
