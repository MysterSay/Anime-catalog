...
__name(normalizeGoogleHref, "normalizeGoogleHref");
...
__name(extractGoogleResultUrls, "extractGoogleResultUrls");
...
__name(fetchGoogleCandidates, "fetchGoogleCandidates");
...
async function handleSourceSearchApi(request) {
  ...
  const previewTasks = searched.flatMap((group) => group.urls.map((url2) => ({ site: group.site, url: url2 })));
  ...
  const groups = searched.map((group) => ({
    ...
    query: group.query,
    error: group.error,
    items: group.urls.map((url2) => previewMap.get(`${group.site.domain}|${url2}`) || { site: group.site.domain, url: url2, title: fallbackTitleFromUrl(url2), image: "", imageSource: "page" })
  }));
  return json({ ok: true, query: q, groups, total: groups.reduce((sum, group) => sum + group.items.length, 0) });
}
__name(handleSourceSearchApi, "handleSourceSearchApi");
...
var worker_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/source-search") return await handleSourceSearchApi(request);
      ...
      if (url.pathname === "/api/version") return json({ ok: true, version: "yoru-v4.4-google-source-core-ingest-2026-08-11", addTitle: true, googleSourceSearch: true, sourceSites: AUTHORITY_SITES, pythonCore: env.CORE_PROCESS_URL || CORE_PROCESS_URL, coreJsonIngest: true, ingestEndpoint: "/api/ingest", mediaRepair: true });
...
//# sourceMappingURL=bundledWorker-0.9312438718923478.mjs.map
