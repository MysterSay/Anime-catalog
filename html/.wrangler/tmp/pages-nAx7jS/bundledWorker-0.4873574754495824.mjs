...
__name(mapPageSummary, "mapPageSummary");
    ...
    for (const [name, prop] of Object.entries(source?.properties || {})) {
      ...
      if (normalizeName(name) === normalizeName(NOTION_PROPERTIES.group)) {
        ...
        if (prop?.type === "multi_select") {
          for (const option of prop.multi_select?.options || []) if (option?.name) groups.add(option.name);
        }
        if (prop?.type === "status") {
          for (const option of prop.status?.options || []) if (option?.name) groups.add(option.name);
        }
      }
    }
  }
...
__name(titlePropertyValue, "titlePropertyValue");
...
__name(booleanPropertyValue, "booleanPropertyValue");
...
__name(fileUploadPageValue, "fileUploadPageValue");
...
__name(notionMultipartFetch, "notionMultipartFetch");
...
__name(handleAnimeUploadApi, "handleAnimeUploadApi");
...
__name(chooseAvailable, "chooseAvailable");
...
__name(mergedSiteLinks, "mergedSiteLinks");
...
__name(handleMergeAnimeApi, "handleMergeAnimeApi");
...
var worker_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/version") return json({ ok: true, version: "yoru-v5.7-notion-favorites-2026-08-13", addTitle: true, googleSourceSearch: true, googleSearchMode: "vercel-python-core", sourceSites: AUTHORITY_SITES, catalogs: SITE_PROPERTIES, pythonCoreSearch: env.CORE_SEARCH_URL || CORE_SEARCH_URL, pythonCoreProcessStream: env.CORE_PROCESS_STREAM_URL || CORE_PROCESS_STREAM_URL, pythonCoreProcessFull: env.CORE_PROCESS_FULL_URL || CORE_PROCESS_FULL_URL, pythonCoreProcess: env.CORE_PROCESS_URL || CORE_PROCESS_URL, progressProtocol: "ndjson-v1", coreJsonIngest: true, ingestEndpoint: "/api/ingest", mediaRepair: true, mergeTitles: true, mergeEndpoint: "/api/anime/merge", deleteTitles: true, editTitles: true, uploadEndpoint: "/api/anime/upload", extensionContext: "/api/extension/context", aliasLookupV2: true, editorMediaMode: "url", favoriteStorage: "notion-checkbox", likedStorage: "notion-checkbox" });
...
//# sourceMappingURL=bundledWorker-0.4873574754495824.mjs.map
