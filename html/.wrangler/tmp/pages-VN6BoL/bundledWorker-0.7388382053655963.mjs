...
function buildCreateProperties(media, payload, translated, siteLinks, source) {
  ...
  if (!properties[NOTION_PROPERTIES.viewed]) properties[NOTION_PROPERTIES.viewed] = { number: Number(payload.viewed || 0) || 0 };
...
async function handleAnimeApi(request, env) {
  ...
  if (request.method === "PATCH") {
    ...
    if (Object.prototype.hasOwnProperty.call(body, "viewed")) {
      const nextViewed = Math.max(0, Number(body.viewed) || 0);
      const name = mapped.viewedEntry?.[0] || NOTION_PROPERTIES.viewed;
...
function buildCoreProperties(payload, source, { includeAddedAt = true } = {}) {
  ...
  properties[NOTION_PROPERTIES.viewed] = { number: Number(data.viewed || 0) || 0 };
...
//# sourceMappingURL=bundledWorker-0.7388382053655963.mjs.map
