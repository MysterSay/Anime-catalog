...
const GOOGLE_CX_STORAGE_KEY = 'yoru-google-cx';
let googleCseReadyPromise = null;
let googleCseCx = '';
let googleCsePending = null;
...
function sourceResultTile(item, site, index) {
  ...
  return `
    <button class="discover-result source-discover-result" type="button"
      ...
      <img src="${escapeHtml(item.image || FALLBACK_IMAGE)}" alt="${escapeHtml(title)}" />
...
function renderGoogleSourceGroups(groups) {
  ...
  if (!safeGroups.length) {
    discoverResults.innerHTML = '<div class="discover-empty">Пошук не повернув жодної групи.</div>';
    return;
  }
  ...
  discoverResults.innerHTML = `
    <div class="google-source-groups">
      ${safeGroups.map((group, groupIndex) => {
        ...
        const open = groupIndex === 0 || (groupIndex > 0 && !safeGroups.slice(0, groupIndex).some(prev => (prev.items || []).length));
        return `
  ...
  setDiscoverStatus(total ? `Знайдено сторінок: ${total}. Обери правильний тайтл.` : 'Google не повернув сторінок тайтлів на цих трьох сайтах.', total ? 'ok' : '');
}

function normalizeCx(value) {
  return String(value || '').trim().replace(/^['"]|['"]$/g, '');
}

async function resolveGoogleCx() {
  const local = normalizeCx(localStorage.getItem(GOOGLE_CX_STORAGE_KEY));
  if (local) return local;
  try {
    const response = await fetch('/api/google-config', { headers: { Accept: 'application/json' }, cache: 'no-store' });
    const payload = await response.json().catch(() => ({}));
    if (response.ok && normalizeCx(payload.cx)) return normalizeCx(payload.cx);
  } catch {}
  return '';
}

function showGoogleCxSetup(queryText = '') {
  discoverResults.innerHTML = '';
  discoverSetup.classList.remove('hidden');
  discoverSetup.innerHTML = `
    <div class="google-cx-setup">
      <div>
        <span class="section-kicker">GOOGLE SEARCH</span>
        <h3>Потрібен Search engine ID (cx)</h3>
        <p>Google блокує серверне читання сторінки /search кодом 429. Для стабільного пошуку сайт використовує офіційний Programmable Search Element у твоєму браузері.</p>
      </div>
      <label class="google-cx-field">
        <span>Search engine ID</span>
        <input id="googleCxInput" type="text" placeholder="Наприклад: 0123456789:abcdef" autocomplete="off" />
      </label>
      <div class="google-cx-actions">
        <button id="saveGoogleCx" class="primary-btn" type="button">Зберегти й повторити пошук</button>
        <a class="secondary-btn" href="https://programmablesearchengine.google.com/controlpanel/create" target="_blank" rel="noopener noreferrer">Створити Google PSE ↗</a>
      </div>
      <p class="google-cx-hint">У Google PSE додай: <b>myanimelist.net</b>, <b>anilist.co</b>, <b>shikimori.io</b>. CX не є паролем і зберігається лише в цьому браузері.</p>
    </div>`;
  setDiscoverStatus('Налаштуй Google Search engine ID один раз — після цього пошук працюватиме без Cloudflare → Google scraping.', 'error');
  const input = document.getElementById('googleCxInput');
  const save = document.getElementById('saveGoogleCx');
  input?.focus();
  save?.addEventListener('click', async () => {
    const cx = normalizeCx(input?.value);
    if (!cx || cx.length < 6 || /\s/.test(cx)) {
      setDiscoverStatus('Введи коректний Search engine ID (cx).', 'error');
      return;
    }
    localStorage.setItem(GOOGLE_CX_STORAGE_KEY, cx);
    googleCseReadyPromise = null;
    googleCseCx = '';
    discoverSetup.classList.add('hidden');
    discoverSetup.innerHTML = '';
    if (queryText) {
      await executeDiscoverSearch(queryText);
    } else {
      setDiscoverStatus('Google Search engine ID збережено.', 'ok');
    }
  });
}

function clientHostMatches(host, domain) {
  const h = String(host || '').toLowerCase().replace(/^www\./, '');
  const d = String(domain || '').toLowerCase().replace(/^www\./, '');
  return h === d || h.endsWith(`.${d}`);
}

function authorityResultAllowed(urlValue, site) {
  try {
    const url = new URL(urlValue);
    return ['http:', 'https:'].includes(url.protocol) && clientHostMatches(url.hostname, site.domain) && site.path.test(url.pathname);
  } catch { return false; }
}

function normalizeCseResult(result) {
  const url = String(result?.url || '').trim();
  const title = String(result?.titleNoFormatting || result?.title || '').replace(/<[^>]+>/g, '').trim();
  return { url, title };
}

function ensureGoogleCse(cx) {
  const normalized = normalizeCx(cx);
  if (!normalized) return Promise.reject(new Error('GOOGLE_CX_MISSING'));
  if (googleCseReadyPromise && googleCseCx === normalized) return googleCseReadyPromise;
  if (googleCseReadyPromise && googleCseCx && googleCseCx !== normalized) {
    return Promise.reject(new Error('GOOGLE_CX_CHANGED_RELOAD'));
  }

  googleCseCx = normalized;
  googleCseReadyPromise = new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('Google Programmable Search не завантажився за 20 секунд.')), 20000);

    window.__gcse = {
      parsetags: 'explicit',
      initializationCallback: function() {
        try {
          const host = document.getElementById('googleCseHost');
          if (!host) throw new Error('Не знайдено контейнер Google Search.');
          google.search.cse.element.render({
            div: host,
            tag: 'searchresults-only',
            gname: 'authorityGoogle',
            attributes: {
      ...
      searchCallbacks: {
        web: {
          ready: function(gname, query, promos, results, resultsDiv) {
            if (gname !== 'authorityGoogle') return false;
            if (resultsDiv) resultsDiv.innerHTML = '';
            const pending = googleCsePending;
            if (pending) {
              googleCsePending = null;
              clearTimeout(pending.timer);
              pending.resolve((Array.isArray(results) ? results : []).map(normalizeCseResult));
            }
            return true;
          },
        },
    ...
    const old = document.querySelector('script[data-yoru-google-cse]');
    if (old) old.remove();
    const script = document.createElement('script');
    script.async = true;
    script.dataset.yoruGoogleCse = '1';
    script.src = `https://cse.google.com/cse.js?cx=${encodeURIComponent(normalized)}`;
    script.onerror = () => {
      clearTimeout(timer);
      reject(new Error('Не вдалося завантажити Google Programmable Search Element. Перевір блокувальник реклами або CX.'));
    };
    document.head.appendChild(script);
  });
  return googleCseReadyPromise;
}

async function runGoogleCseQuery(query) {
  const cx = await resolveGoogleCx();
  if (!cx) throw new Error('GOOGLE_CX_MISSING');
  await ensureGoogleCse(cx);
  const element = window.google?.search?.cse?.element?.getElement('authorityGoogle');
  if (!element) throw new Error('Google Search Element не ініціалізовано.');
  if (googleCsePending) throw new Error('Попередній Google-пошук ще не завершився.');

  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      if (googleCsePending) googleCsePending = null;
      reject(new Error('Google не повернув результати за 20 секунд.'));
    }, 20000);
    googleCsePending = { resolve, reject, timer, query };
    try {
...
async function fetchDirectPagePreviews(groups) {
  ...
  try {
    ...
    const previews = Array.isArray(payload.items) ? payload.items : [];
    const previewMap = new Map(previews.map(item => [`${item.site}|${item.requestedUrl || item.url}`, item]));
    return groups.map(group => ({
      ...group,
      items: (group.items || []).map(item => {
        const preview = previewMap.get(`${group.site}|${item.url}`);
        return preview ? { ...item, ...preview, title: preview.title || item.title } : item;
      }),
    }));
  } catch (error) {
    console.warn('Source page preview failed', error);
    return groups;
  }
}
...
async function searchGoogleSources(queryText) {
  ...
  for (let i = 0; i < AUTHORITY_SEARCH_SITES.length; i++) {
    const site = AUTHORITY_SEARCH_SITES[i];
    const query = `site:${site.domain} "${clean}"`;
    setDiscoverStatus(`Google ${i + 1}/${AUTHORITY_SEARCH_SITES.length}: ${site.label}…`, 'loading');
    try {
      const raw = await runGoogleCseQuery(query);
      const seen = new Set();
      const items = [];
      for (const result of raw) {
        if (!authorityResultAllowed(result.url, site)) continue;
        const parsed = new URL(result.url);
        parsed.hash = '';
        const url = parsed.href;
        if (seen.has(url)) continue;
        seen.add(url);
        items.push({ site: site.domain, url, title: result.title || url, image: '', imageSource: 'page' });
        if (items.length >= 8) break;
      }
      groups.push({ site: site.domain, label: site.label, query, error: '', items });
    } catch (error) {
      if (error?.message === 'GOOGLE_CX_MISSING') throw error;
      groups.push({ site: site.domain, label: site.label, query, error: error?.message || 'Google search error', items: [] });
    }
  }

  return { ok: true, query: clean, groups: await fetchDirectPagePreviews(groups) };
}
...
async function executeDiscoverSearch(q) {
  ...
  discoverSetup.classList.add('hidden');
  discoverSetup.innerHTML = '';
  discoverResults.innerHTML = '<div class="discover-loading"><span class="detail-loader"></span><span>Google: MyAnimeList · AniList · Shikimori…</span></div>';
  setDiscoverStatus('Google-пошук виконується у браузері; Cloudflare більше не звертається до google.com/search.', 'loading');
  try {
    const payload = await searchGoogleSources(q);
    renderGoogleSourceGroups(payload.groups || []);
  } catch (error) {
    console.error(error);
    if (error?.message === 'GOOGLE_CX_MISSING') {
      showGoogleCxSetup(q);
      return;
    }
    if (error?.message === 'GOOGLE_CX_CHANGED_RELOAD') {
      discoverResults.innerHTML = '';
      setDiscoverStatus('CX змінено. Онови сторінку (Ctrl+F5) і повтори пошук.', 'error');
      return;
    }
    discoverResults.innerHTML = '';
    setDiscoverStatus(error.message || 'Помилка Google-пошуку.', 'error');
  }
}
